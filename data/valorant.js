(() => {
  ...
  async function cargarStreamValorant() {
    ...
    try {
      ...
      // Filtrar únicamente streamers en directo jugando a Valorant
      const streamsJuego = streams.filter(s => s.game_id === GAME_ID_VALORANT);
      ...
      if (listaStreamers) {
        if (streamsJuego.length > 0) {
          listaStreamers.innerHTML = streamsJuego.map(s => {
            ...
            return `
              <button onclick="setStream('${s.user_login}')" class="w-full flex items-center justify-between p-2.5 rounded-xl bg-slate-900/60 hover:bg-slate-800 transition border border-transparent hover:border-red-500/30 text-left group">
                <div class="flex items-center gap-3">
                  <div class="w-8 h-8 rounded-full bg-red-900/50 border border-red-500/40 flex items-center justify-center font-bold text-xs text-red-300">
                    ${s.user_name.substring(0, 2).toUpperCase()}
                  </div>
                  <div>
                    <div class="text-xs font-bold text-slate-200 group-hover:text-red-300 transition">${s.user_name}</div>
